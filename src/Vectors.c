...
void Vec3_Transform(Vec3* result, const Vec3* a, const struct Matrix* mat) {
	...
	float x = a->X * mat->Row0.X + a->Y * mat->Row1.X + a->Z * mat->row3.X + mat->row4.X;
	float y = a->X * mat->Row0.Y + a->Y * mat->Row1.Y + a->Z * mat->row3.Y + mat->row4.Y;
	float z = a->X * mat->Row0.Z + a->Y * mat->Row1.Z + a->Z * mat->row3.Z + mat->row4.Z;
...
void Matrix_Scale(struct Matrix* result, float x, float y, float z) {
	*result = Matrix_Identity;
	result->Row0.X = x; result->Row1.Y = y; result->row3.Z = z;
}
