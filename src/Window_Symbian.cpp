...
extern "C" cc_bool Launcher_DoFrame(void);
extern "C" void Game_DoFrame(void);

static CC_INLINE void RunLauncher(void) {
	gameRunning = false;
	Launcher_Run();
}
...
void CCContainer::ConstructL(const TRect& aRect, CAknAppUi* aAppUi) {
	...
	iPeriodic->Start(100, 100, TCallBack(CCContainer::LoopCallBack, this));
}
