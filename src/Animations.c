...
#define LIQUID_ANIM_MAX 24
#else
#define LIQUID_ANIM_MAX 32
#endif
	...
	for (y = 0; y < size; y++)
	{
		for (x = 0; x < size; x++)
		{
			...
			*ptr = BitmapCol_Make(
				color * 120.0f + 135.0f,
				color * color * 255.0f,
				color * color * color * color * 150.0f,
				255);
...
static void Animations_ReadDescription(struct Stream *stream, const cc_string *path)
{
	...
	for (;;)
	{
		...
		if (res)
		{
			Logger_SysWarn2(res, "reading from", path);
			break;
		}

		if (!line.length || line.buffer[0] == '#')
			continue;
		count = String_UNSAFE_Split(&line, ' ', parts, ANIM_MIN_ARGS);
		if (count < ANIM_MIN_ARGS)
		{
			Chat_Add1("&cNot enough arguments for anim: %s", &line);
			continue;
		}

		if (!Convert_ParseUInt8(&parts[0], &tileX) || tileX >= ATLAS2D_TILES_PER_ROW)
		{
			Chat_Add1("&cInvalid anim tile X coord: %s", &parts[0]);
			continue;
		}
		if (!Convert_ParseUInt8(&parts[1], &tileY) || tileY >= ATLAS2D_MAX_ROWS_COUNT)
		{
			Chat_Add1("&cInvalid anim tile Y coord: %s", &parts[1]);
			continue;
		}
		if (!Convert_ParseUInt16(&parts[2], &data.frameX))
		{
			Chat_Add1("&cInvalid anim frame X coord: %s", &parts[2]);
			continue;
		}
		if (!Convert_ParseUInt16(&parts[3], &data.frameY))
		{
			Chat_Add1("&cInvalid anim frame Y coord: %s", &parts[3]);
			continue;
		}
		if (!Convert_ParseUInt16(&parts[4], &data.frameSize) || !data.frameSize)
		{
			Chat_Add1("&cInvalid anim frame size: %s", &parts[4]);
			continue;
		}
		if (!Convert_ParseUInt16(&parts[5], &data.statesCount))
		{
			Chat_Add1("&cInvalid anim states count: %s", &parts[5]);
			continue;
		}
		if (!Convert_ParseUInt16(&parts[6], &data.frameDelay))
		{
			Chat_Add1("&cInvalid anim frame delay: %s", &parts[6]);
			continue;
		}

		if (anims_count == Array_Elems(anims_list))
		{
			Chat_AddRaw("&cCannot show over 512 animations");
			return;
		}
...
static void Animations_Update(int texLoc, struct Bitmap *bmp, int stride)
{
	...
	if (tex)
		Gfx_UpdateTexture(tex, 0, dstY, bmp, stride, Gfx.Mipmaps);
}
...
static void Animations_Apply(struct AnimationData *data)
{
	...
	loc = data->texLoc;
#ifndef CC_BUILD_WEB
	if (loc == LAVA_TEX_LOC && useLavaAnim)
		return;
	if (loc == WATER_TEX_LOC && useWaterAnim)
		return;
#endif
	...
	frame.scan0 = anims_bmp.scan0 + data->frameY * anims_bmp.width + (data->frameX + data->state * size);
...
static void Animations_Validate(void)
{
	...
	for (i = 0; i < anims_count; i++)
	{
		...
		else
		{
			...
			if (data->texLoc == LAVA_TEX_LOC && !alwaysLavaAnim)
				useLavaAnim = false;
			if (data->texLoc == WATER_TEX_LOC && !alwaysWaterAnim)
				useWaterAnim = false;
			continue;
		}
