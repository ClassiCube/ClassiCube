...
namespace ClassicalSharp.GraphicsAPI {
	...
	public class DirectXApi : IGraphicsApi {
		...
		int GetNumPrimitives( int vertices, DrawMode mode ) {
			switch( mode ) {
		...
		public DirectXApi( Device device, Game game ) {
			this.device = device;
			this.game = game;
		...
		public override void DrawVertices( DrawMode mode, OpenTK.Vector3[] vertices, int count ) {
			...
			device.DrawUserPrimitives( modeMappings[(int)mode], GetNumPrimitives( count, mode ), vertices );
		}
